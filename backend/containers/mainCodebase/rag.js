...
class RAGManager {
  ...
  async augmentWithWeb(query, personality = null) {
    ...
    const enhancedQuery = this._buildSearchQuery(query, personality);
    const keyTerms = this._extractKeyConcepts(query);
    try {
      const pType = this._detectPersonalityType(personality);
      console.log(`🔎 RAG augmentWithWeb personality=${pType} query="${query}" ⇒ enhanced="${enhancedQuery}"`);
    } catch (_) {}
    ...
    try {
      const preview = (hits || []).slice(0, 5).map(h => {
        try { return new URL(h.url).hostname; } catch { return h.url; }
      });
      console.log(`📚 RAG ranked top hosts: ${preview.join(', ')}`);
    } catch (_) {}
    ...
    if (!hits.length) return { sources: [], contextBlock: '', searchQuery: enhancedQuery, keyTerms };
    ...
    const contextBlock = `SEARCH QUERY: ${enhancedQuery}\nKEY TERMS: ${keyTerms.join(', ')}\n\nSOURCES\n${lines.join('\n')}`;
    return { sources: hits, contextBlock, searchQuery: enhancedQuery, keyTerms };
  }
   ...
   */
  _buildSearchQuery(query, personality = null) {
    ...
    // Personality-based search bias (only when not explicitly asking for research papers)
    if (!wantsResearchPapers) {
      if (personalityType === 'casual') {
        // Prefer entertaining, accessible sources
        searchQuery += ' OR site:reddit.com OR site:youtube.com OR meme OR "funny explanation" OR "simple guide"';
      } else if (personalityType === 'technical') {
        // Prefer technical docs and Q&A
        searchQuery += ' OR site:stackoverflow.com OR site:github.com OR documentation OR "best practices"';
      } else if (personalityType === 'business') {
        // Prefer business/industry reports
        searchQuery += ' OR site:bloomberg.com OR site:hbr.org OR site:mckinsey.com OR "industry report" OR "market analysis"';
      } else if (personalityType === 'academic') {
        // Light academic bias even without explicit request
        searchQuery += ' OR site:.edu OR site:nature.com OR site:science.org';
      }
    }

    // Strong commerce bias for fitness/gym coaches
    if (/(fitness|gym|workout|coach|trainer)/i.test((personality?.description || '') + ' ' + (personality?.tone || '') + ' ' + (personality?.communicationStyle || ''))) {
      if (!wantsResearchPapers) {
        searchQuery += ' OR buy OR price OR review OR site:amazon.com OR site:decathlon.com OR site:nike.com OR site:adidas.com';
      } else {
        // When research requested, still allow gear reviews as supplemental
        searchQuery += ' OR review OR site:examine.com';
      }
    }
  ...
  _filterAndRankSources(sources, query, personality = null) {
    ...
    // Personality hard scope gate: casual/funny bots avoid academic unless explicitly requested
    if (personalityType === 'casual' && !wantsResearchPapers) {
      const isAcademicHost = (u) => /arxiv\.org|scholar\.google\.com|pubmed|ieee\.org|nature\.com|science\.org|springer\.com|wiley\.com|elsevier\.com|acm\.org|\.edu\b/i.test(u || '');
      const academicTiers = new Set(['Government/Agency','Intergovernmental Agency','Academic','Peer-reviewed']);
      const filteredScoped = scoredSources.filter(s => !isAcademicHost(s.url) && !academicTiers.has(s.reliability));
      const pool = filteredScoped.length ? filteredScoped : scoredSources.slice(0, 1);
      return pool
